...
public class CnfAdapterRest {
    ...
    public DeferredResult<ResponseEntity> upgrade(@PathVariable("instanceID") String instanceId,
                                                  @RequestBody InstanceUpgradeRequest upgradeRequest) {
        ...
        new Thread(() -> {
            ...
            String upgradeResponse = null;
            try {
                upgradeResponse = instanceUpgradeService.upgradeInstance(instanceId, upgradeRequest);
            } catch (Exception e) {
                logger.error("END - upgrade process failed", e);
            }
            callbackClient.sendPostCallback(upgradeRequest.getCallbackUrl(), upgradeResponse);
        }).start();
